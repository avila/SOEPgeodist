...
schooldata %>%
  mutate(
    ...
    art_reduziert = case_when(
      art_reduziert == 1 ~ "gs",
      art_reduziert == 2 ~ "mit_gym_os",
      art_reduziert == 3 ~ "ohne_gym_os"
    ),

    priv_schule_typ = case_when(
      priv_schule_typ == 1 ~ "Frei",
      priv_schule_typ == 2 ~ "Kirchlich",
      priv_schule_typ == 3 ~ "Montessori",
      priv_schule_typ == 4 ~ "Waldorf"
    ),
    ...
    bundesland = case_when(
      ...
      bundesland == 7  ~ "hessen",
      bundesland == 8  ~ "meckpomm",
      bundesland == 9  ~ "niedersachsen",
      bundesland == 10 ~ "nrw",
      bundesland == 11 ~ "rp",
      bundesland == 12 ~ "saarland",
      bundesland == 13 ~ "sachsen",
      bundesland == 14 ~ "sachsenanhalt",
      bundesland == 15 ~ "sh",
      bundesland == 16 ~ "thueringen"
    )
  )
...
# Calculations  -----------------------------------------------------------
if (FALSE) {
  # make sure packages are also loaded
  load(fname_Rdata)
}
